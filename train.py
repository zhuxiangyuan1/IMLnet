from imdn_model.architecture import *
...
def parse_args():
    ...
    parser.add_argument('--n_epochs', type=int, default=1800, help='number of epochs to train')
...
def train(train_loader, cfg):
    ...
    for idx_epoch in range(cfg.n_epochs):
        ...
        for idx_iter, (HR_left, _, LR_left, LR_right) in enumerate(train_loader):
            ...
            HR_left, HR_right,LR_left, LR_right  = Variable(HR_left).to(cfg.device),Variable(_).to(cfg.device), Variable(LR_left).to(cfg.device), Variable(LR_right).to(cfg.device)

            SR_left,SR_right = net(LR_left,LR_right)

            ### loss_SR
            # loss_SR = 0.84*(1-ms_ssim_loss(SR_left, HR_left))+0.16*criterion_L1(SR_left, HR_left)
        ...
        if idx_epoch % 1 == 0:
            ...
            with torch.no_grad():
                for idx_iter, (HR_left, _, LR_left, LR_right) in enumerate(test_loader):
                    ...
                    SR_left,_ = net(LR_left,LR_right)
                ...
                if psnr_now >= psnr_max:
                    ...
                    print(cfg.dataset + '    PSNR_now:', "%.4f" % psnr_now, '    SSIM_now:', "%.4f" % ssim_now,
                          '    PSNR_max:', "%.4f" % psnr_max, '   current_epoch:', idx_epoch + 1, '   max_epoch:',
